...
public:
    ...
    {
        //与驱动通信
        void* hooked_function = GetProcAddress(LoadLibrary(L"win32u.dll"), "NtQueryCompositionSurfaceStatistics");
        if (hooked_function == NULL)
        {
            std::cout << "[-] driver::send_instruction get NtQueryCompositionSurfaceStatistics failed" << std::endl;
            return FALSE;
        }
